...
func TestProbeAndReturn(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			EfiVarFs = tt.path
			if _, err := ProbeAndReturn(); err != ErrFsNotMounted {
...
func TestGet(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			attr, data, err := vfs.Get(tt.vd.Name, tt.vd.GUID)
			if err != nil {
				if err != tt.wantErr {
...
func TestSet(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := vfs.Set(tt.vd.Name, tt.vd.GUID, tt.attr, tt.data); err != nil {
				if err != tt.wantErr {
...
func TestRemove(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := vfs.Remove(tt.vd.Name, tt.vd.GUID); err != nil {
				if err != tt.wantErr {
					// Needed as some errors include changing tmp directory names
					if !strings.Contains(err.Error(), tt.wantErr.Error()) {
						t.Errorf("Want: %v, Got: %v", tt.wantErr, err)
